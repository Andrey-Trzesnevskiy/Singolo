$(document).ready(function(){
	var lArr = $('<i data-direction="right">&lt;</i>').css('left','-10px');
	var rArr = $('<i data-direction="left">&gt;</i>').css('right','-10px');
	...
	$('.slider i')
        ...
        $('.slider_item').first().clone().appendTo('.slider_items');
				$('.slider_item').first().animate({marginLeft:'-=200px'},300,function(){ 
					$(this).remove();
				});
			} else if (direction == 'left') {
				$('.slider_item').last().clone().prependTo('.slider_items').css('margin-left','-160px');
				$('.slider_items').animate({paddingLeft:'+=200px'},300,function(){ 
